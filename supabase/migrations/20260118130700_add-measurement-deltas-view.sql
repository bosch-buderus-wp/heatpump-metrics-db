-- Create measurement_deltas view
-- This view shows all individual measurements with computed deltas from the previous measurement
-- Allows users to identify and delete problematic measurements
CREATE OR REPLACE VIEW measurement_deltas AS
  ...
  SELECT 
    ...
    CASE 
      WHEN LAG(m.thermal_energy_kwh) OVER w IS NOT NULL 
           AND m.thermal_energy_kwh > LAG(m.thermal_energy_kwh) OVER w
    ...
    CASE 
      WHEN LAG(m.electrical_energy_kwh) OVER w IS NOT NULL 
           AND m.electrical_energy_kwh > LAG(m.electrical_energy_kwh) OVER w
    ...
    CASE 
      WHEN LAG(m.thermal_energy_heating_kwh) OVER w IS NOT NULL 
           AND m.thermal_energy_heating_kwh > LAG(m.thermal_energy_heating_kwh) OVER w
    ...
    CASE 
      WHEN LAG(m.electrical_energy_heating_kwh) OVER w IS NOT NULL 
           AND m.electrical_energy_heating_kwh > LAG(m.electrical_energy_heating_kwh) OVER w
